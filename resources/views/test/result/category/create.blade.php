    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body row">
                <form role="form" id="createForm" action="{{route('test.result.category.store')}}" method="POST" style="width: 100%; display: contents;">

                    @csrf

                    <div class="form-group col-md-12">
                        <label>Test Item Name</label>
                        <select name="test_item_id" class="form-control">
                            <option  hidden></option>
                            @foreach($testItems as $item)
                                <option value="{{ $item->id }}">{{ $item->name }}</option>
                            @endforeach
                        </select>
                        <div id="test_item_id_error" class="text-danger"></div>
                    </div>
